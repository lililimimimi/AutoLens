// AutoLens — components/chat/ChatAgentChain.tsx

const GREEN = "#5a7a5a";

const agentSteps = [
  { key: "router", desc: "意图识别" },
  { key: "memory", desc: "记忆读取" },
  { key: "research", desc: "RAG 检索" },
  { key: "reflection", desc: "合规检查" },
];

interface ChatAgentChainProps {
  currentStep: number; // -1 未开始，0-3 进行中，4+ 完成
}

export default function ChatAgentChain({ currentStep }: ChatAgentChainProps) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 12, padding: "20px 24px" }}>
      ...
      >
        <div style={{ fontSize: 16, fontWeight: 600 }}>客服 Agent 链路</div>
        {done && (
          <div
            style={{
              fontSize: 12,
